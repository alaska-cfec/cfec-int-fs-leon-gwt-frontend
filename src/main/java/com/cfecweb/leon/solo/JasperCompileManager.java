 ...
 * {@link net.sf.jasperreports.engine.design.JRCompiler}.
 * 
...
{
    private static final JasperReportsContext JASPER_REPORTS_CONTEXT = DefaultJasperReportsContext.getInstance();
    private static final JRPropertiesUtil JR_PROPERTIES_UTIL = JRPropertiesUtil.getInstance(JASPER_REPORTS_CONTEXT);
	...
	private static JRCompiler getJavaCompiler()
	{
		...
		try 
		{
			...
			compiler = new JRJdtCompiler(JASPER_REPORTS_CONTEXT);
		}
		catch (Exception e)
		{
		}
		...
		if (compiler == null)
		{
			try 
			{
				...
				compiler = new JRJdk13Compiler(JASPER_REPORTS_CONTEXT);
			}
			catch (Exception e)
			{
			}
		...
		if (compiler == null)
		{
			compiler = new JRJavacCompiler(JASPER_REPORTS_CONTEXT);
		}
	...
	private static JRCompiler getCompiler(JasperDesign jasperDesign) throws JRException
	{
		...
		if (compilerClassName == null || compilerClassName.trim().length() == 0)
		{
			...
			compilerClassName = JR_PROPERTIES_UTIL.getProperty(JRCompiler.COMPILER_PREFIX + language);
			if (compilerClassName == null || compilerClassName.trim().length() == 0)
			{
	...
	{
		return JR_PROPERTIES_UTIL.getProperty(JRCompiler.COMPILER_CLASS);
	}
	...
	public static void main(String[] args) throws Exception {
		JasperCompileManager test = new JasperCompileManager();
		JasperCompileManager.compileReportToFile("/webapps/jrxml/LEON/sformPermitSub.jrxml", "/webapps/jrxml/LEON/sformPermitSub.jasper");
	}
}
